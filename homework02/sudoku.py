...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
    """
    return [[values.pop(0) for ind_2 in range(n)] for ind_1 in range(n)]
...
def find_empty_positions(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.Tuple[int, int]]:
    ...
    (2, 0)
    """
    for ind_1, row in enumerate(grid):
        for ind_2, element in enumerate(row):
            if element == ".":
                return ind_1, ind_2
    return None
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    return all_values - (block | row | col)
    ...
    for ind_1 in range(len(solution)):
        for ind_2 in range(len(solution)):
            if (
                ...
                or set(get_block(solution, (ind_1, ind_2))) != all_values_num
            ):
                return False
    return True
